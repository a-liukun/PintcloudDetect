  ...
  for (int j = 0; j < v_clouds.size(); j++)
    {
      ...
      pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZ> cloud_in_color_h(cloudPtr);  //赋予显示点云的颜色,随机
...
void Directfilter(pcl::PointCloud<pcl::PointXYZ>::Ptr &origin, double a, double b)
{
 ...
 /*pcl::PassThrough<pcl::PointXYZ> pass;  //创建滤波器对象
  ...
  pass.setFilterLimits(-b, -0);        //设置滤波范围
  pass.setFilterLimitsNegative(false);   //保留范围
  pass.filter(*origin);
  cout << "直通滤波"
...
//检测台
bool Detecttai(pcl::PointCloud<pcl::PointXYZ>::Ptr origin)
{
  pcl::PassThrough<pcl::PointXYZ> pass;	 //创建滤波器对象
  pass.setInputCloud(origin);			 //设置待滤波的点云
  pass.setFilterFieldName("x");			 //设置在x轴方向上进行滤波
  pass.setFilterLimits(0, 10);			 //设置滤波范围
  pass.setFilterLimitsNegative(false);	 //保留范围
  pass.filter(*origin);					 //滤波并存储
  pass.setInputCloud(origin);			 //设置待滤波的点云
  pass.setFilterFieldName("y");			 //设置在y轴方向上进行滤波
  pass.setFilterLimits(-2, 1);		 //设置滤波范围
  pass.setFilterLimitsNegative(false);	 //保留范围
  pass.filter(*origin);

  pass.setInputCloud(origin);			 //设置待滤波的点云
  pass.setFilterFieldName("z");			 //设置在z轴方向上进行滤波
  pass.setFilterLimits(-1, 0.5);			 //设置滤波范围
  pass.setFilterLimitsNegative(false);	 //保留范围
  pass.filter(*origin);					 //滤波并存储
  cout << "直通滤波"<< " " << origin->points.size() << endl;
  if (STEP == 'B')
  {
    showdianyungreen(origin); //截取区域
  }
  ...
  seg.setAxis(Eigen::Vector3f(1, 0, 0));
  //该角度为平面与轴的多方位夹角，结果为整个所设置的角度中内点最多的
  seg.setEpsAngle(0.2);     //  如果要找出很垂直与z轴的立面角度设置为0.2，最大不要超过0.785
  seg.setMaxIterations(500);  //设置为500次
  seg.setDistanceThreshold(0.1);  //单位米，点到模型的距离
  seg.setInputCloud(cloud_ptr);
  seg.setInputNormals(cloud_normals);
  ...
  Radiusoutlier(cloud_cluster2,0.1,50);

  cout << "拟合台面平云点的个数" << cloud_cluster2->points.size() << endl;
  if (STEP == 'C')
  {
    showdianyungreen(cloud_cluster1);  //原始点云
    showdianyunred(cloud_cluster2);    //台面
  }
  
  pcl::PointXYZ min_p1, max_p1;
  Eigen::Vector3f c1, c;
  pcl::getMinMax3D(*cloud_cluster2, min_p1, max_p1);
  cout << "拟合的台面的高度约为：" << max_p1.z-min_p1.z << endl;
  /*
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_clusterorgin(new pcl::PointCloud<pcl::PointXYZ>);  //保存原始点云
  //cloud_clusterorgin = origin;
  // 1.直通滤波
  Directfilter(origin, 2, 2);
 ...
 // }
  ...
  Eigen::Vector3f c1, c;
  pcl::getMinMax3D(*cloud_cluster2, min_p1, max_p1);
  float taiheight=max_p1.z-min_p1.z;

  if(cloud_cluster2->points.size()>=1000&&taiheight<=1)
  {
    cout << "拟合台面平云点的个数" << cloud_cluster2->points.size() << endl;
    cout << "拟合的台面的高度约为：" <<taiheight<< endl;
...
void chatterCallback(const sensor_msgs::PointCloud2::ConstPtr cloud)
{
  ...
  {
    cout << "----------------正在进行一次台面检测------------------" << endl;
    flagtaiG = Detecttai(preorigin);
    return;
  }
  else
    return;
...
void DrawCube(Eigen::Vector3d p1,Eigen::Vector3d p2,Eigen::Vector3d p3,ros::Time s)
{
    ...
    marker.ns = "basic_shapes_tai";
      ...
      cout << "发送第" << i << "个台marker" << endl;
}

float r_w, r_x, r_y, r_z,p_x, p_y, p_z;
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
  // cout<<"ros::time"<<odomMsg->header.stamp<<endl;
  if(!flagtaiG)
  {
    return;
  }
...
int main(int argc, char **argv)
{
  ...
  marker_pub = nh.advertise<visualization_msgs::Marker>("visualization_marker_tai", 1);
